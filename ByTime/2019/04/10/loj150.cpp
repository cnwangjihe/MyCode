...
int Wn[2][24];
int a[MAXN],b[MAXN],c[MAXN],t1[MAXN],t2[MAXN],t3[MAXN];
char s[MAXN];
int n,k;
...
//	freopen("loj150.out","w",stdout);
	...
	for (int i=0;i<n;i++)
		IO.scan(a[i]);
	copy(a,a+n,c);
	polynomial_sqrt(n,a,b,t1,t2);
	polynomial_inverse(n,b,a,t1);
	polynomial_integral(n,a,b);
	polynomial_exp(n,b,a,t1,t2);
	c[0]=2;
	for (int i=0;i<n;i++)
		a[i]=Add(c[i],Mod(-a[i]));
	polynomial_ln(n,a,b,t1);
	b[0]++;
	polynomial_qpow(n,b,a,t1,t2,t3,k);
	polynomial_derivation(n,a,b);
	for (int i=0;i<n-1;i++)
		IO.print(b[i],' ');
	IO.println();
	return 0;
